...
#include "CLevel2BonusGameFlow.h"
#include "CLevel2BonusEnemySpawner.h"
#include "Level2BonusGlobals.h"
...
static void InitHUD()
{
    ...
    CHUDMgr::SetLevelTitle("The Bridge: Bonus");
}
//-----------------------------------------------------------------------------
CLevel2BonusGameFlow::CLevel2BonusGameFlow(uint _uiSkill)
{
	...
	m_poEnemySpawner = new CLevel2BonusEnemySpawner(_uiSkill);
	m_poCinematic = CFECinematicLoader::poLoad("data/cinematics/level2c");
...
//-----------------------------------------------------------------------------
CLevel2BonusGameFlow::~CLevel2BonusGameFlow()
{
}
//-----------------------------------------------------------------------------
void CLevel2BonusGameFlow::Reset()
{
	CEntityMgr::Reset();
...
void CLevel2BonusGameFlow::Start()
{
...
void CLevel2BonusGameFlow::Restart()
{
	goGameGlobals.m_poCamera->Spawn(CFEVect2(100,0));
...
//-----------------------------------------------------------------------------
bool CLevel2BonusGameFlow::bFinished()
{
    return( uiGetState() == L1CGFS_FINISH );
}
//-----------------------------------------------------------------------------
void CLevel2BonusGameFlow::GenericGameLoop(FEReal _rDeltaT)
{
...
//-----------------------------------------------------------------------------
void CLevel2BonusGameFlow::Update(FEReal _rDeltaT)
{
	switch (uiGetState())
	{
		case L1CGFS_NONE:
...
//-----------------------------------------------------------------------------
void CLevel2BonusGameFlow::OnEnterState(uint _uiState)
{
	switch (_uiState)
	{
		case L1CGFS_INITIAL_CINEMATIC:
...
//-----------------------------------------------------------------------------
void CLevel2BonusGameFlow::OnExitState(uint _uiState, uint _uiNewState)
{
	switch (_uiState)
	{
		case L1CGFS_INITIAL_CINEMATIC:
