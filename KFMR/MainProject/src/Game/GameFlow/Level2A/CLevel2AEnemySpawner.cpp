...
#include <stdio.h>
//-----------------------------------------------------------------------------
const uint MAX_ENEMY_TYPES = 3;
const uint MAX_ENEMIES[MAX_ENEMY_TYPES] = {16,3,6};
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CLevel2AEnemySpawner::CLevel2AEnemySpawner(uint _uiSkill)
{
...
//-----------------------------------------------------------------------------
CLevel2AEnemySpawner::~CLevel2AEnemySpawner()
{
    for (uint j=0;j<MAX_ENEMY_TYPES;j++)
    {
    	for (uint i=0;i<MAX_ENEMIES[j];i++)
...
//-----------------------------------------------------------------------------
void CLevel2AEnemySpawner::Reset()
{
    for (uint j=0;j<MAX_ENEMY_TYPES;j++)
    {
    	for (uint i=0;i<MAX_ENEMIES[j];i++)
...
//-----------------------------------------------------------------------------
void CLevel2AEnemySpawner::SpawnEnemy(uint _uiType,bool _bLeft)
{
...
//-----------------------------------------------------------------------------
void CLevel2AEnemySpawner::Update(FEReal _rDeltaT)
{
	m_rTime += _rDeltaT;
	...
	if ((goGameGlobals.m_poCamera->oGetPos().x < RIGHT_SPAWN_LIMIT) && (m_poThrowers[0]==NULL))
	{
		...
		if (m_rRightSideSpawnTime<=_0r)
		{
			...
			if (((m_uiRightSideSpawns+1) % 5) == 0 ||		// Enano Saltarín
...
//-----------------------------------------------------------------------------
void CLevel2AEnemySpawner::NotifyDeath(CEntity* _poEnt)
{
