...
static CFEPSDefinition	oPS;
static CFEParticle		oPart;
...
void CGameLoop::Init(CFERenderer* _poRnd,CFEInput* _poInput)
{
	...
	// Load the game level
 	/*
 	goGameGlobals.m_poMap = CFEMapLoader::poLoad("data/level/level");
	...
	goGameGlobals.m_poPlayer->Spawn(PLAYER_POS);
...
void CGameLoop::Render()
{
	m_poRnd->BeginScene();
		...
		m_poRnd->SetMaterial(NULL);
		m_poRnd->ResetCamera();	
		m_poRnd->RenderQuad(0,0,m_poRnd->uiGetScreenVWidth(),m_poRnd->uiGetScreenVHeight(),0,0,1,1,CFEColor(0.5f,0.5f,0.5f,1));

		m_poRnd->TranslateCamera(goGameGlobals.m_poCamera->oGetPos().x,goGameGlobals.m_poCamera->oGetPos().y + CAMERA_YOFS);
