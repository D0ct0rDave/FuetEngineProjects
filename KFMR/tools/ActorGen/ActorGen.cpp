    ...
    poFWriter->OpenSection("SkelAnim");
        ...
        for (uint uiAction=0;uiAction<m_oActions.size();uiAction++)
        {
            ...
            poFWriter->OpenSection("Action",uiAction);
                ...
                poFWriter->AddVar("NumNodeActions",4);
                ...
                poFWriter->OpenSection("NodeAction",2);
                    ...
                    poFWriter->OpenSection("PosFunc");

						poFWriter->AddVar("WrapMode",sGetActionFuncPlayMode(m_oActions[uiAction]->m_sActionName));
                        poFWriter->AddVar("NumKeyFrames",m_oActions[uiAction]->m_oHeadCoords.size());

                        float fKeyFrameTime = 0.0;
                        for (uint uiKeyFrame=0;uiKeyFrame<m_oActions[uiAction]->m_oHeadCoords.size();uiKeyFrame++)
                        {
                            poFWriter->OpenSection("KeyFrame",uiKeyFrame);
                                
                                poFWriter->AddVar("x", m_oActions[uiAction]->m_oHeadCoords[uiKeyFrame].x);
                                poFWriter->AddVar("y", m_oActions[uiAction]->m_oHeadCoords[uiKeyFrame].y);
                                poFWriter->AddVar("Time",fKeyFrameTime);
                                poFWriter->AddVar("LerpFunc", "linear" );

                            poFWriter->CloseSection();
                            fKeyFrameTime += fFrameTime;
                        };
               ...
               poFWriter->OpenSection("NodeAction",3);
                    ...
                    poFWriter->OpenSection("PosFunc");
						...
						poFWriter->AddVar("WrapMode",sGetActionFuncPlayMode(m_oActions[uiAction]->m_sActionName));
                        poFWriter->AddVar("NumKeyFrames",m_oActions[uiAction]->m_oBodyCoords.size());

                        fKeyFrameTime = 0.0;
                        for (uint uiKeyFrame=0;uiKeyFrame<m_oActions[uiAction]->m_oBodyCoords.size();uiKeyFrame++)
                        {
                            poFWriter->OpenSection("KeyFrame",uiKeyFrame);
                                
                                poFWriter->AddVar("x", m_oActions[uiAction]->m_oBodyCoords[uiKeyFrame].x);
                                poFWriter->AddVar("y", m_oActions[uiAction]->m_oBodyCoords[uiKeyFrame].y);
                                poFWriter->AddVar("Time",fKeyFrameTime);
                                poFWriter->AddVar("LerpFunc", "linear" );
...
void CActorGenerator::Process(const fipImage& oImg)
{
	...
	for (int iPass=0;iPass<2;iPass++)
	{
		...
		{
			case 0:
				// First pass is for head pos.
				oKeyColor = CFEColor(1,0,0,1);
			break;

			case 1:
				// First pass is for body pos.
