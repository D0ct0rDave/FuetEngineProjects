...
void CBinConfigFileWriter::Write()
{
    ...
    {
        // Write dictionary
        for (unsigned int i=0;i<m_poData->m_sDictionary.size();i++)
...
void CBinConfigFileWriter::WriteIdentifier(const std::string& _sIdentifier)
{
    ...
    int iPos = -1;
    for (int i=0;i<m_poData->m_sDictionary.size();i++)
    {
        if (m_poData->m_sDictionary[i] == _sIdentifier)
        {
            iPos = i;
            break;
        }
    ...
    if (iPos == -1)
    {
        iPos = m_poData->m_sDictionary.size();
        m_poData->m_sDictionary.push_back(_sIdentifier);        
    }

    fwrite(&iPos,4,1,m_poFD);
