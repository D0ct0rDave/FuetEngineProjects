...
void CEditorAboutBox::CreateGUIControls()
{
	...
	wxBitmap WxStaticBitmap1_BITMAP(actor_editor_about_xpm);
	...
	WxMemo1->AppendText(wxT("FuetEngine ActorEditor\n"));
	wxString wxVersion = wxT(ActorEditorVersion::FULLVERSION_STRING);
	WxMemo1->AppendText( wxT("version ") + wxVersion + wxT("\n") );
	
	wxString wxDate =	wxString(wxT(ActorEditorVersion::DATE)) + wxString(wxT("-")) +
						wxString(wxT(ActorEditorVersion::MONTH)) + wxString(wxT("-")) +
						wxString(wxT(ActorEditorVersion::YEAR)) + wxString(" (") + wxString( wxT(__TIME__) ) + wxString(")");
	...
	WxMemo2->AppendText(wxT("\n"));
	WxMemo2->AppendText(wxT("wxWidgets \n"));
	WxMemo2->AppendText(wxT("http://www.wxwidgets.org/\n"));
	WxMemo2->AppendText(wxT("\n"));
	WxMemo2->SetFocus();
