...
class CFERenderer
{
    public:
		...
		private:
			...
			void PushRenderPrim(uint _uiPrimType);
			void PushRenderPrimQuad();
            void PushRenderPrimMesh(CFEVect2* _poVX,CFEVect2* _poUV,CFEColor* _poVC,uint _uiNumVXs = 0,unsigned short* _pusIdx = NULL);

            // Flushes the contents of the render primitive list.
            void FlushRenderPrims(bool _bClearPrimList = true);

            // Flushes the contents of the render packet list.
            void FlushRenderPackets(bool _bClearPrimList = true);

            // Build a list of render packets to be sent to the device renderer.
            void BuildRenderPackets();

            // Update the transformation matrix after camera or transform modifications.
            void UpdateViewTransform();

			void GenericRenderPrimitive(CRenderPrim* _poRP);
