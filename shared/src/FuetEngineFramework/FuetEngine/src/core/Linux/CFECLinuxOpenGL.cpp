...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
FEHandler CFECystem::Graphics::hInit(FEHandler _hParam)
{
...
// ----------------------------------------------------------------------------
void CFECystem::Graphics::Finish(FEHandler _hRenderer)
{
    Check(_hRenderer != NULL,"NULL Renderer Handler");
    DestroyRenderingContext() ;
}
// ----------------------------------------------------------------------------
void CFECystem::Graphics::BeginScene(FEHandler _hRenderer)
{
    if (FECglobals.m_poCurrentRenderer != _hRenderer)
    {
        ...
        glXMakeCurrent( FECglobals.m_poCurrentRenderer->m_poDisplay,
                        FECglobals.m_poCurrentRenderer->m_oWin,
                        FECglobals.m_poCurrentRenderer->m_hGLRC);
    }
    ...
    glScalef    ( 2.0f/(float)FECglobals.m_uiScrVWidth,-2.0f/(float)FECglobals.m_uiScrVHeight,1.0f);
    CHECKERROR();
...
// ----------------------------------------------------------------------------
void CFECystem::Graphics::EndScene()
{
    glFinish();
    glXSwapBuffers( FECglobals.m_poCurrentRenderer->m_poDisplay,
                    FECglobals.m_poCurrentRenderer->m_oWin);
}
// ----------------------------------------------------------------------------
#endif
