...
// ----------------------------------------------------------------------------
#include "../FEConfig.h"
#if (PROFILE == USE_PHYSIXFSPROFILE)
// ----------------------------------------------------------------------------
#include <stdlib.h>
#include <math.h>
#include "physfs.h"
...
namespace CFECore { namespace Profile {
	...
	void Initialize();
	FEBool m_bInitialized = false;
	CFEString sGetProfileFilename(const CFEString& _sFilename);

	FEBool bDumpSaveData(PHYSFS_File* f,FEPointer _pData, uint _uiSize);
...
// ----------------------------------------------------------------------------
void CFECore::Profile::Initialize()
{
	if (! PHYSFS_isInit())
	{
		if (! PHYSFS_init(NULL))
	...
	const char* szUserDir = PHYSFS_getUserDir();
	CFEString sAppDir = CFEString::sFormat(".coderchild/%s",FECglobals.m_sApplicationName.szString());

	PHYSFS_setWriteDir(szUserDir);

	// create the application output directory
	PHYSFS_mkdir(sAppDir.szString());
	
	/// set the final write directory. Needs to be real. i.e. not "virtual"
	sAppDir = CFEString::sFormat("%s.coderchild/%s",szUserDir,FECglobals.m_sApplicationName.szString());
	sAppDir = CFEStringUtils::sNormalizePath(sAppDir);

	if (PHYSFS_setWriteDir(sAppDir.szString()) != 0)
		// Add the new dir to the search paths
		PHYSFS_mount(sAppDir.szString(),NULL,1);

	m_bInitialized = true;
}
// ----------------------------------------------------------------------------
CFEString CFECore::Profile::sGetProfileFilename(const CFEString& _sFilename)
{
...
void Encrypt(unsigned char* _pucDstData,unsigned char* _pucSrcData, uint _uiSrcSize)
{
	for (uint i=0;i<_uiSrcSize;i++)
		_pucDstData[i] = _pucSrcData[i] ^ 0x5a;
}
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
FEBool CFECore::Profile::bDumpSaveData(PHYSFS_File* fd,FEPointer _pData, uint _uiSize)
{
	...
	FEPointer pNewData = CFECore::Mem::pAlloc(uiTotalSize);
		...
		// Write to file
		uint uiRes = PHYSFS_write(fd,pNewData,1,uiTotalSize);
...
FEBool CFECore::Profile::bSave(FEPointer _pData, uint _uiSize)
{
	...
	OpenMessageBox(_uiSize,oTicks,GENERIC_SAVE_MESSAGE,GENERIC_SAVE_MESSAGE_OPTIONS,false);
		...
		PHYSFS_File* fd = PHYSFS_openWrite(sFilename.szString());
		if (fd != NULL)
		{
...
FEBool CFECore::Profile::bLoad(FEPointer _pData, uint _uiSize)
{
	...
	// Open file
	PHYSFS_File* fd = PHYSFS_openRead(sFilename.szString());
	if (fd == NULL)
	{
    ...
    FEBool res = false;
    uint uiReadBytes = PHYSFS_read(fd,pLoadedData,1,uiTotalSize);
	PHYSFS_close(fd);
...
uint CFECore::Profile::uiGetVersion()
{
    ...
    uint uiReadBytes = PHYSFS_read(fd,&oProfileHeader,1,sizeof(TProfileHeader));
    PHYSFS_close(fd);
