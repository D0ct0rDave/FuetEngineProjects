...
// ----------------------------------------------------------------------------
#include "../FEConfig.h"
#if ((GRAPHICS == USE_OPENGL_STEREO) && (TARGETPLATFORM == USE_DEVELOPMENT))
// ----------------------------------------------------------------------------
#include <windows.h>
#include <stdlib.h>
#include <math.h>
...
class CRendererHandler
{
    public:
        ...
        CRendererHandler(unsigned short _usReservedPrims) :	m_hDC(NULL),
								...
								m_rParallaxFact(_0r),
								m_rParallaxSlider(_0r),
								m_bStereoRender(false)
		{
		
			m_rParallaxCurve.InsertKeyFrame(0.0     ,0.0f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.010653,0.1f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.019531,0.2f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.027040,0.3f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.033482,0.4f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.039062,0.5f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.043945,0.6f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.048253,0.7f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.052083,0.8f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.055509,0.9f,KFLF_LERP);
			m_rParallaxCurve.InsertKeyFrame(0.058593,1.0f,KFLF_LERP);
		...
		FEReal			m_rParallaxSlider;		// Current Parallax (between 0..1)		
		FEReal			m_rParallaxFact;		// Parallax factor
		FEBool			m_bStereoRender;
		CFEKFBFunc<FEReal> m_rParallaxCurve;
...
// ----------------------------------------------------------------------------
static FEReal			m_rParallaxSlider;		// Current Parallax (between 0..1)
static FEReal			m_rParallaxFact;		
static FEBool			m_bStereoRender;
// ----------------------------------------------------------------------------
namespace CFECore { namespace Graphics { 
	...
	void SaveTGA(const CFEString& _sFilename,FEPointer _pData,uint _uiWidth,uint _uiHeight);
...
FEHandler CFECore::Graphics::hInit(FEHandler _hParam)
{
	...
	poRnd->m_rParallaxFact  = m_rParallaxFact;
	poRnd->m_rParallaxSlider= m_rParallaxSlider;
	poRnd->m_bStereoRender	= m_bStereoRender;

	// These need to be re setup to have effect on the next renderer handler initialization.
	m_rParallaxFact		= _0r;
	m_rParallaxSlider	= _0r;
	m_bStereoRender		= false;
	GLint major,minor;
...
void CFECore::Graphics::EndScene()
{
	...
	if (FECglobals.m_poCurrentRenderer->m_uiPrims > 0)
	{
		...
		float fParallaxFact = FECglobals.m_poCurrentRenderer->m_rParallaxFact;
		FECglobals.m_poCurrentRenderer->m_rParallaxFact = _0r;
	
			FlushRenderList();
	
		FECglobals.m_poCurrentRenderer->m_rParallaxFact = fParallaxFact;
		if (FECglobals.m_poCurrentRenderer->m_uiLightPrims > 0)
		{
...
/*
			...
			CHECKERROR();
				...
				glColor4f(_1r,_1r,_1r,_05r);
				CHECKERROR();
				glTexCoord2f(_0r,_0r);
				CHECKERROR();
				glVertex2f(-_1r,-_1r);
				CHECKERROR();

				glColor4f(_1r,_1r,_1r,_1r);
				CHECKERROR();
				glTexCoord2f(_1r,_0r);
				CHECKERROR();
				glVertex2f(_1r,-_1r);
				CHECKERROR();

				glColor4f(_1r,_1r,_1r,_0r);
				CHECKERROR();
				glTexCoord2f(_1r,_1r);
				CHECKERROR();
				glVertex2f(_1r,_1r);
				CHECKERROR();
	...
	switch (_eBlendMode)
    {
		...
		*/
		...
		case BM_REVEAL:
			...
			glAlphaFunc(GL_ALWAYS,0.0f);
			glEnable(GL_BLEND);
			glBlendFuncSeparateEXT(GL_ZERO,GL_DST_ALPHA,GL_ZERO,GL_ZERO);
		break;
		*/
    }
}
...
{
	float fPFact =   FECglobals.m_poCurrentRenderer->m_rParallaxSlider
					*FECglobals.m_poCurrentRenderer->m_rParallaxCurve.oGetValue(FECglobals.m_rCurDepth)
					*FECglobals.m_poCurrentRenderer->m_rParallaxFact
					/**700.0f
					*100.0f*/;
...
// ----------------------------------------------------------------------------
void CFECore::Graphics::SaveTGA(const CFEString& _sFilename,FEPointer _pData,uint _uiWidth,uint _uiHeight)
{
	#pragma pack(push,1)
	...
	if (hHnd != NULL)
	{
		...
		CFECore::File::uiWrite(hHnd,_pData,_uiWidth*_uiHeight*3);
		CFECore::File::Close(hHnd);
		#else
		fwrite(&tga,TGA_HEADER_SIZE,1,hHnd);
		fwrite(_pData,_uiWidth*_uiHeight*3,1,hHnd);
		fclose(hHnd);
		#endif
	}
}
...
FEBool CFECore::Graphics::bSetProperty(const CFEString& _sProperty,FEPointer _pParam)
{
    ...
    else if (_sProperty == "STEREO_FACTOR")
	{
		m_rParallaxFact =  *(FEReal*)_pParam;
		return(true);
	}
	else if (_sProperty == "STEREO_SLIDER")
	{
		m_rParallaxSlider =  *(FEReal*)_pParam;
		return(true);
	}
	else if (_sProperty == "STEREO_RENDER")
	{
		m_bStereoRender = (_pParam!=NULL);
		return(true);
	}
	else if (_sProperty == "SCREENSHOT")
    {
		if (FECglobals.m_poCurrentRenderer != NULL)
		{		
			FEReal	rParallaxFact = FECglobals.m_poCurrentRenderer->m_rParallaxFact;
			uint uiWidth  = FECglobals.m_poCurrentRenderer->m_uiScrWidth;
			uint uiHeight = FECglobals.m_poCurrentRenderer->m_uiScrHeight;
			FEPointer pData = CFECore::Mem::pAlloc(uiWidth*uiHeight*3);
			...
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
					...
					SaveTGA(*(CFEString*)_pParam,pData,uiWidth,uiHeight);

			::SwapBuffers( FECglobals.m_poCurrentRenderer->m_hDC );

			// Then if stereo enabled render left and right images
			if (FECglobals.m_poCurrentRenderer->m_bStereoRender)
			{
				FECglobals.m_poCurrentRenderer->m_rParallaxFact = rParallaxFact;
				CFEString sFile = *(CFEString*)_pParam + "-r";
				for (uint i=0;i<2;i++)
				{
					glClearColor(0,0,0,1);
					glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
						...
						glReadPixels(0,0,uiWidth,uiHeight,GL_RGB,GL_UNSIGNED_BYTE,pData);
						SaveTGA(sFile,pData,uiWidth,uiHeight);

					::SwapBuffers( FECglobals.m_poCurrentRenderer->m_hDC );
				
					// 2 times inverted... no need to backup
					FECglobals.m_poCurrentRenderer->m_rParallaxFact = -rParallaxFact;
					sFile = *(CFEString*)_pParam + "-l";
				}
			}
