...
class CRendererHandler
{
    public:
								...
								m_usIdx(NULL)
		{
			...
			{
				m_oVXBufs[i]	= (float*)CFECore::Mem::pAlloc( uiRoundUp(uiMaxVXs*sizeof(float)*3),MEM_ALIGNMENT);
...
namespace CFECore { namespace Graphics { 
	...
	void INT_RenderMesh(unsigned short* _pusIdx,float* _poVXs,CFEVect2* _poUVs,const CFEColor& _oColor,uint _uiPoints);
	void INT_RenderMesh(unsigned short* _pusIdx,float* _poVXs,CFEVect2* _poUVs,CFEColor* _poVCs,uint _uiPoints);
	void INT_RenderLine(float* _poVXs,CFEColor* _poVCs,uint _uiPrims);
...
void CFECore::Graphics::RenderLine(const CFEVect2& _oIni,const CFEVect2& _oEnd,const CFEColor& _oColor)
{
    ...
    float*	  poDstVX = &poRnd->m_oVX[uiIdx*3];
                    ...
                    const CFEColor& _oColor)
{
    ...
    // Set UVs
	if (poRP->m_oRS.m_eBlendMode != BM_REVEAL)
	{
		poDstUV[0].x = _oIUV.x;
		poDstUV[0].y = _oIUV.y;
	...
	else
	{
		...
		// process UV
		for (uint v=0;v<4;v++)
			poDstUV[v] = oMVP.Transform(  CFEVect2(poDstVX[v*3+0],poDstVX[v*3+1]) );
	}
...
void CFECore::Graphics::RenderQuadList(CFEVect2* _poVXs,CFEVect2* _poUVs,CFEColor* _poVCs,uint _uiQuads)
{
	...
	for (uint i=0;i<_uiQuads;i++)
	{
		...
		poDstVX[0] = _poVXs[0].x;
		poDstVX[1] = _poVXs[0].y;
		poDstVX[2] = _0r;
		
		poDstVX[3] = _poVXs[1].x;
		poDstVX[4] = _poVXs[1].y;
		poDstVX[5] = _0r;
		
		poDstVX[6] = _poVXs[2].x;
		poDstVX[7] = _poVXs[2].y;
		poDstVX[8] = _0r;
		
		poDstVX[9]  = _poVXs[3].x;
		poDstVX[10] = _poVXs[3].y;
		poDstVX[11] = _0r;
                    ...
                    const CFEColor& _oColor,FEReal _rAlpha)
{
	...
	if (poRnd->m_oRenderState.m_eBlendMode == BM_MAGICMARKER)
	{
	...
	*/
	...
	if (_rAngle == _0r)
	{
		...
		poDstVX[6]   = _1r - _oPivot.x;
		poDstVX[7]   = _1r - _oPivot.y;
		poDstVX[8]   = _0r;
		...
		poDstVX[9]  = poDstVX[0];
		poDstVX[10] = poDstVX[7];
		poDstVX[11] = _0r;

		for (uint i=0;i<4;i++)
		{
			poDstVX[i*3+0] *= _oScale.x;
			poDstVX[i*3+0] += _oPos.x;
			
			poDstVX[i*3+1] *= _oScale.y;
			poDstVX[i*3+1] += _oPos.y;
		}
	}
	else
	{
		...
		for (uint i=0;i<4;i++)
		{
			...
			// 3
			CFEVect2 oPos = CFEVect2(poDstVX[i*3+0],poDstVX[i*3+1]);
			poDstVX[i*3+0] = oPos.x*rCosA - oPos.y*rSinA;
			poDstVX[i*3+1] = oPos.x*rSinA + oPos.y*rCosA;
...
void CFECore::Graphics::FlushRenderList()
{
	...
	for (uint i=0;i<poRnd->m_oRL.size();i++)
	{
		...
		if (poElem->m_poIniPrim->m_oRS.m_eBlendMode != eBM)
		{
			...
			EnableRender2Texture( (eBM == BM_LIGHTMARKER) || (eBM == BM_LIGHTSABER_STEP_2_INT));

			INT_SetBlendMode(poElem->m_poIniPrim->m_oRS.m_eBlendMode);
			CHECKERROR();
		}

		if (poElem->m_poIniPrim->m_oRS.m_eBlendMode != BM_REVEAL)
		{
	...
	switch (_eBlendMode)
    {
		...
		*/
		...
		case BM_LIGHTSABER_STEP_1_INT:
		{
			...
			glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_RGB,	GL_MODULATE);			// Texture A * Vertex RGB
			glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB,	GL_TEXTURE);
...
// ----------------------------------------------------------------------------
void CFECore::Graphics::INT_RenderMesh(unsigned short* _pusIdx,float* _poVXs,CFEVect2* _poUVs,const CFEColor& _oColor,uint _uiPoints)
{
// return;
/*
	if (_poUVs)
...
*/
	...
	if (_poVXs)
	{
		...
		CHECKERROR();
		glVertexPointer(3, GL_FLOAT, 3*sizeof(GLfloat), _poVXs);
		CHECKERROR();
	}
	else
	{
...
// ----------------------------------------------------------------------------
void CFECore::Graphics::INT_RenderMesh(unsigned short* _pusIdx,float* _poVXs,CFEVect2* _poUVs,CFEColor* _poVCs,uint _uiPoints)
{
	/*
	if (_poUVs)
	{
...
// ----------------------------------------------------------------------------
void CFECore::Graphics::INT_RenderLine(float* _poVXs,CFEColor* _poVCs,uint _uiPrims)
{
	glBegin(GL_LINES);
