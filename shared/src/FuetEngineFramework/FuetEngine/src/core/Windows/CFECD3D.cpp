...
// ----------------------------------------------------------------------------
#include "../FEConfig.h"
#if ((GRAPHICS == USE_DIRECT3D) && (TARGETPLATFORM == USE_DEVELOPMENT))
// ----------------------------------------------------------------------------
...
HRESULT hLoadTexture(const CFEString& _sFilename,TMaterialHandler* _poMH)
{
	...
	{		
		...
		sLocalFilename = CFECore::Local::sGetLocalID(LID_COMMON) + "/" + _sFilename;
		sFilename = sLocalFilename;

		// try to load a sample in memory.
		hr = D3DXCreateTextureFromFile(FECglobals.m_poCurrentRenderer->m_poD3DDevice,sLocalFilename.szString(), &_poMH->m_poTex);
	
		if (FAILED(hr))
		{		
			// try first localized version of the file.
			sLocalFilename = CFECore::Local::sGetLocalID() + "/" + _sFilename;
			sFilename = sLocalFilename;
	...
	{
		hr = D3DXCreateTextureFromFile(FECglobals.m_poCurrentRenderer->m_poD3DDevice,_sFilename.szString(), &_poMH->m_poTex);
	}
	
	// TMaterialHandler
    if (! FAILED(hr))
    {
		CFECImageInfo* poII = ImageLib::poLoadImage((char*)sFilename.szString());
		if (poII != NULL)
		{
...
{	
    ...
    hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->Clear(0,//Number of rectangles to clear, we're clearing everything so set it to 0
	...
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->BeginScene();
	
	
	D3DXMATRIX oViewMatrix;
	D3DXMatrixIdentity(&oViewMatrix);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTransform(D3DTS_VIEW,&oViewMatrix);

	D3DXMATRIX oProjMatrix;
	D3DXMatrixIdentity(&oProjMatrix);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTransform(D3DTS_PROJECTION, &oProjMatrix );
	...
	D3DXMATRIX oS;
	D3DXMatrixScaling(&oS,2.0f/(FEReal)FECglobals.m_uiScrVWidth,-2.0f/(FEReal)FECglobals.m_uiScrVHeight,1.0);
	D3DXMATRIX oT;
	D3DXMatrixTranslation(&oT,-1.0,1.0,0.0);
	oWorldMatrix = oS * oT;

	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTransform(D3DTS_WORLD,&oWorldMatrix);
    */
	...
	D3DXMatrixTranslation(&tmp, -0.5f, FECglobals.m_uiScrVHeight+0.5f, 0.0f);
	D3DXMatrixMultiply(&oWorldMatrix, &oWorldMatrix, &tmp);
	D3DXMatrixOrthoOffCenterLH(&tmp, 0, (float)FECglobals.m_uiScrVWidth, 0, (float)FECglobals.m_uiScrVHeight, 0.0f, 1.0f);
	D3DXMatrixMultiply(&oWorldMatrix, &oWorldMatrix, &tmp);

	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTransform(D3DTS_PROJECTION,&oWorldMatrix);
}
// ----------------------------------------------------------------------------
void CFECore::Graphics::EndScene()
{
	...
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->EndScene();

	//Show the results
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->Present(NULL,  //Source rectangle to display, NULL for all of it
...
{
    FECglobals.m_uiScrVWidth = _uiScrVWidth;
...
{
    FECglobals.m_uiScrVHeight = _uiScrVHeight;
...
{
	if (FECglobals.m_poCurrentRenderer == NULL) return(NULL);
...
void CFECore::Graphics::SetBlendMode(EFEBlendMode _eBlendMode)
{
    ...
    {
        case BM_NONE:
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,FALSE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_ONE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_ZERO);
        break;
        ...
        case BM_ALPHA:
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,TRUE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_SRCALPHA);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_INVSRCALPHA);
        break;

        case BM_ADD:
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,TRUE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_ONE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_ONE);
        break;
        ...
        case BM_MULT:
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_ALPHABLENDENABLE,TRUE);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_SRCCOLOR);
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_DESTCOLOR);
        break;
    }
}
// ----------------------------------------------------------------------------
void CFECore::Graphics::SetMaterial(FEHandler _hMaterial)
{
    ...
    if (_hMaterial == NULL)
    {
		hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTexture(0,NULL);
        return;
    }
    ...
    // Bind the material
    hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTexture(0,poTexHnd->m_poTex);
    
    
    
    hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetSamplerState(0, D3DSAMP_ADDRESSU,  D3DTADDRESS_MIRRORONCE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetSamplerState(0, D3DSAMP_ADDRESSV,  D3DTADDRESS_MIRRORONCE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);



	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_COLORARG1,D3DTA_TEXTURE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_COLORARG2,D3DTA_DIFFUSE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_COLOROP,	D3DTOP_MODULATE);

	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_ALPHAARG1,D3DTA_TEXTURE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_ALPHAARG2,D3DTA_DIFFUSE);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetTextureStageState(0,D3DTSS_ALPHAOP,  D3DTOP_MODULATE);   
}
// ----------------------------------------------------------------------------
...
void CFECore::Graphics::RenderQuads(CFEVect2* _poVX,CFEVect2* _poUV,CFEColor* _poVC,uint _uiNumQuads)
{
	...
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetFVF(gsoVertexFormatInfo[uiVFI].m_dwFmt);

	void* pVD;
	hr = FECglobals.m_poCurrentRenderer->m_poVB[uiVFI]->Lock(0,0,&pVD,0);
		...
		switch (uiVFI)
		{
			case 0:
			{
				...
				for (uint i=0;i<4;i++)
				{
					...
					poVD[i].x = _poVX[i].x;
					poVD[i].y = _poVX[i].y;
					poVD[i].z = FECglobals.m_rCurDepth;
					// poVD[i].w = 1.0;
			...
			case 1:
			{
				...
				for (uint i=0;i<4;i++)
				{
					...
					poVD[i].x = _poVX[i].x;
					poVD[i].y = _poVX[i].y;
					poVD[i].z = 0.5f;// .FECglobals.m_rCurDepth;
					// poVD[i].w = 1.0;
	...
	hr = FECglobals.m_poCurrentRenderer->m_poVB[uiVFI]->Unlock();
    
//    hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN,2,pVD,gsoVertexFormatInfo[uiVFI].m_dwSize);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetStreamSource(0,FECglobals.m_poCurrentRenderer->m_poVB[uiVFI],0,gsoVertexFormatInfo[uiVFI].m_dwSize);
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->DrawPrimitive(D3DPT_TRIANGLEFAN,0,2);
}
// ----------------------------------------------------------------------------
...
void CFECore::Graphics::RenderLine(const CFEVect2& _oIni,const CFEVect2& _oEnd,const CFEColor& _oColor)
{
    ...
    oVX[0].x = _oIni.x;
    oVX[0].y = _oIni.y;
    oVX[0].z = FECglobals.m_rCurDepth;
    oVX[0].color = dwColor(_oColor);
    
    oVX[1].x = _oEnd.x;
    oVX[1].y = _oEnd.y;
    oVX[1].z = FECglobals.m_rCurDepth;
    oVX[1].color = oVX[0].color;
    
	hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->SetFVF(gsoVertexFormatInfo[0].m_dwFmt);
    hr = FECglobals.m_poCurrentRenderer->m_poD3DDevice->DrawPrimitiveUP(D3DPT_LINELIST,1,oVX,gsoVertexFormatInfo[0].m_dwSize);
}
// ----------------------------------------------------------------------------
